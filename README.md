...
- **💫 动态交互感知 (UI)**：带有动态节点状态机展示，通过 HTTP Server-Sent Events (SSE) 长连接，实现整个 LangGraph 思维链路和状态转移的酷炫可视化实时呈现。
- **💾 轻量级持久化**：前置拦截生成操作，当有效报告被最终确认后隐蔽化沉淀至网页内存中，防止误触刷新丢失（前端 local Storage 缓存支持）。

## 📸 界面展示
![界面展示](screenshots/image.png)

## 🛠️ 技术栈

- **引擎之心**：`LangGraph`, `LangChain`, `OpenAI SDK`
- **后端服务**：`Python 3.9+`, `FastAPI`, `Pydantic`, `Uvicorn`
- **前端可视化**：`React 18`, `TypeScript`, `Vite`, `React-Markdown`
...
├── backend/                  # 🧠 大模型研究后端
│   ├── agent/                # 智能体执行中枢
│   │   ├── graph.py          # 路由流转与工作流拓扑构建
│   │   ├── nodes.py          # 具体角色逻辑节点库 (Triage, Orchestrator 等)
│   │   ├── prompts.py        # Master 级系统提示词库
│   │   ├── state.py          # 跨节点全局记忆与状态维持机制
│   │   └── tools.py          # URL提取与聚合搜索工具
│   ├── api/                  # FastAPI 暴露的对外服务网关
│   ├── main.py               # SSE 长连接下发流服务端
│   └── config.py             # 全局环境依赖配置项
├── frontend/                 # 🔭 可视化交互前端
│   ├── src/
│   │   ├── api/              # 流式接驳事件解析引擎
│   │   ├── components/       # 解耦渲染 UI (动态可视化网络 / 结果展示面板)
│   │   ├── App.tsx           # 高阶事件树状总线调度
